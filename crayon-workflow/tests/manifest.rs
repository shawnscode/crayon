...
#[test]
fn load() {
    let manifest = crayon_workflow::manifest::Manifest::find("tests/resources").unwrap();
    ...
    assert_eq!(manifest.types.get("bytes").unwrap(), &Resource::Binary);
    assert_eq!(manifest.types.get("lua").unwrap(), &Resource::Binary);
}