...
mod test {
    ...
    pub fn space() {
...

        {
            ...
            Transform::set_world_position(&mut arena, e3, math::Vector3::new(1.0, 0.0, 0.0))
                .unwrap();
            let disp = Transform::world_position(&mut arena, e4).unwrap();
            assert_eq!(disp, math::Vector3::new(1.0, 0.0, 0.0));

            Transform::set_world_position(&mut arena, e1, math::Vector3::new(1.0, 0.0, 2.0))
                .unwrap();
            let disp = Transform::world_position(&mut arena, e4).unwrap();
    ...
    pub fn hierachy() {
...

        {
            ...
            let t5 = world.fetch::<Transform>(e5).unwrap();
            let t6 = world.fetch::<Transform>(e6).unwrap();
