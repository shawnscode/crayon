...
pub struct GraphicsSystem {
    ...
    views: ObjectPool<Arc<RwLock<ViewStateObject>>>,
    pipelines: ObjectPool<Arc<RwLock<PipelineStateObject>>>,
    vertex_buffers: ObjectPool<Arc<RwLock<VertexBufferObject>>>,
    index_buffers: ObjectPool<Arc<RwLock<IndexBufferObject>>>,
    textures: ObjectPool<Arc<RwLock<TextureObject>>>,
    renderbuffers: ObjectPool<Arc<RwLock<RenderBufferObject>>>,
    framebuffers: ObjectPool<Arc<RwLock<FrameBufferObject>>>,
...
impl GraphicsSystem {
    ...
    pub fn new(window: Arc<window::Window>) -> Result<Self> {
        Ok(GraphicsSystem {
               ...
               views: ObjectPool::new(),
               pipelines: ObjectPool::new(),
               vertex_buffers: ObjectPool::new(),
               index_buffers: ObjectPool::new(),
               textures: ObjectPool::new(),
               renderbuffers: ObjectPool::new(),
               framebuffers: ObjectPool::new(),
               handle_buf: Vec::new(),
    ...
    pub fn run_one_frame(&mut self) -> Result<()> {
        ...
        for handle in self.framebuffers.iter() {
            ...
            // If this framebuffer is owned by `GraphicsSystem` only, then free it.
            if Arc::strong_count(&item) == 1 {
                self.handle_buf.push(handle);
            } else {
